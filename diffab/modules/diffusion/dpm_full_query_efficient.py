...
class FullDPM(nn.Module):
    ...
    def _choose_next_sample_by_weight(self, store_next, metric_sampled, mode="min", beta=None):
        store_v, store_p, store_s = store_next
        ...
        # elif mode  == "random":
        #     selected = 1
        selected = torch.ones(metric_sampled.shape[0], dtype=torch.int64).to(metric_sampled.device)

        num_idx = torch.arange(len(selected), device=selected.device)
        v_next = store_v[selected, num_idx]
        s_next = store_s[selected, num_idx]
        
        has_nan = torch.any(torch.isnan(s_next))
        if torch.sum(has_nan)>0:
            print('s', has_nan)
