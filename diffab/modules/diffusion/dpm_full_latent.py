    ...
    def sample(
        ...
        for t in pbar(range(self.num_steps, 0, -1)):
            if t > Iter:
                ...
                temp_iter = 101 ## 1 latent + DDPM, 101 latent + DDIM
            ...
            for _ in pbar_internal(range(step_sample_times)):  # 1 or 20 (sample by metric)
                if (Best_N == False) and (compute_ddg_step or compute_hydro_step):
            ...
            if compute_ddg_step or compute_hydro_step:
                if Best_N == True:
                    ...
                    selected_noise, selected_s = self._choose_next_sample_latent(store_noise, store_s, metric_sampled, mode=sample_step_mode, temp=temp)
                    v_pred, R_pred, eps_p_pred, c_denoised = self.eps_net(
                        v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=sigma*selected_noise
                        )   # (N, L, 3), (N, L, 3, 3), (N, L, 3)
                    
                    # DDIM
                    v_next = self.trans_rot.denoise(v_t, v_pred, mask_generate, t_tensor, Iter=101)
                    p_next = self.trans_pos.denoise(p_t, eps_p_pred, mask_generate, t_tensor, Iter=101)
