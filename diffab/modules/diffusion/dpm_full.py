...
class EpsilonNet(nn.Module):
    ...
    def forward(self, v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=None, return_res_dim=False):
        """
        Args:
            v_t:    (N, L, 3).
            p_t:    (N, L, 3).
        ...
        if res_feat_noise is not None:
            res_feat = res_feat+res_feat_noise
        ...
        if return_res_dim:
            return v_next, R_next, eps_pos, c_denoised, res_feat.shape
        else:
...
class FullDPM(nn.Module):
    ...
    def _choose_next_sample(self, store_next, metric_sampled, mode="min", beta=None, t=None):
        ...
        row_mins = metric_sampled[num_idx, selected]
        print(f"BestN: all, row_min:{torch.mean(row_mins):.4g}, row_std:{torch.std(row_mins):.4g}")

        return v_next, p_next, s_next
    
    def _choose_next_sample_latent(self, store_noise, store_s, metric_sampled, mode="min", property_weight=None, temp=10):
        print('our latent')
        # Stack tensors
        store_noise = torch.stack(store_noise)
        store_s = torch.stack(store_s)
        # `metric_sampled` is a dictionary with metrics (add up metrics)
        if not property_weight:
            metric_sampled = torch.stack([torch.stack(item).T for item in metric_sampled.values()]).sum(0)  # batch x num samples
        else: 
            ### weight sum of objectives for MOO                
            weights = torch.tensor([property_weight, 1-property_weight]).to(next(iter(metric_sampled.values()))[0].device)  # 权重，形状为 (3,)
            weights = weights.view(-1, 1, 1)
            metric_weighted = torch.stack([torch.stack(item).T for item in metric_sampled.values()])  # batch x num_samples
            metric_sampled = 2 * (metric_weighted * weights).sum(0)  # batch x num_samples 加权求和   
                    
        if mode == 'min':
            _, selected = torch.min(metric_sampled, dim=1)

        elif mode == 'exp':   # Get probabilities (using exponential distribution)
            metric_sampled_min = torch.abs(metric_sampled.min(dim=-1, keepdim=True).values)
            metric_sampled_shift = metric_sampled + metric_sampled_min                    
            lambdas = (1 / metric_sampled_min)
            exp_distrib = lambdas * torch.exp(-temp * lambdas * metric_sampled_shift)
            prob = exp_distrib / (exp_distrib.sum(dim=-1, keepdim=True) + 1e-8)
            selected = self._sample_multinomial(prob)
        elif mode == "softmax":  # Get probabilities (using softmax) and sample                   
            prob = F.softmax(-temp * metric_sampled, dim=-1)
            selected = self._sample_multinomial(prob)
        else:
            raise NotImplementedError
        print(mode)
        ...
        num_idx = torch.arange(len(selected), device=selected.device)
        row_mins = metric_sampled[num_idx, selected]
        # print(f"ID:{t}, all, row_min:{torch.mean(row_mins):.4g}, row_std:{torch.std(row_mins):.4g}")
        selected_noise = store_noise[selected, num_idx]
        selected_s = store_s[selected, num_idx]     
        # metric_weight = F.softmax(-metric_sampled, dim=-1)
        # metric_weight = metric_weight.unsqueeze(2).unsqueeze(3)    
        # metric_weight = metric_weight.permute(1, 0, 2, 3)   
        # selected_noise = torch.sum(metric_weight*store_noise, dim=0).to(store_noise.dtype)

        return selected_noise, selected_s

    def _weight_next_sample_latent(self, store_noise, store_s, metric_sampled, mode="min", property_weight=None, temp=10, noise_dim=None):
        print('our latent weighted tanh')
        # Stack tensors
        store_noise = torch.stack(store_noise)
        store_s = torch.stack(store_s)
        # `metric_sampled` is a dictionary with metrics (add up metrics)
        ...
        metric_sampled_mean = torch.mean(metric_sampled, dim=1, keepdims=True)
        metric_sampled_std = torch.std(metric_sampled, dim=1, keepdims=True)
        metric_sampled_nor = (metric_sampled-metric_sampled_mean)/(metric_sampled_std+1e-8)
        if mode == "tanh":
            metric_sampled_nor = torch.tanh(-metric_sampled_nor)  
        else:
            metric_sampled_nor = torch.softmax(-metric_sampled_nor/0.1, dim=1)
        # metric_sampled_min = torch.min(metric_sampled, dim=1, keepdims=True)[0]
        # metric_sampled_max = torch.max(metric_sampled, dim=1, keepdims=True)[0]
        # metric_sampled_nor = (metric_sampled-metric_sampled_min)/(metric_sampled_max-metric_sampled_min)
        metric_sampled_nor = metric_sampled_nor.transpose(1, 0).unsqueeze(2).unsqueeze(3)
        
        weighted_noise = torch.sum(store_noise * metric_sampled_nor, dim=0)
        weighted_noise = weighted_noise/(((weighted_noise**2).sum(dim=[1, 2], keepdims=True)).sqrt()+1e-6)*(noise_dim**0.5)
        weighted_noise = weighted_noise.to(store_noise.dtype)
        
        return weighted_noise, metric_sampled.squeeze()
    ...
    def sample(
        ...
        sample_step_mode="min", sample_step_num=20, sample_step_period=1, property_weight=0.5,
        normalize=True,
        pbar=False,
        naive=False,
        temp=10,
        ...
        temp_ddg = sample_step_by_ddg
        temp_hydro = sample_step_by_hydro
        temp_num = sample_step_num
        for t in pbar(range(self.num_steps, 0, -1)):
            if t > Iter:
                ...
                sample_step_by_ddg=temp_ddg
                sample_step_by_hydro=temp_hydro
                sample_step_num = temp_num
                if opt_type == 'Latent':
                    temp_iter = 101
                elif opt_type == 'BestN':
                    temp_iter = 1
            ...
            if ((t-1) % sample_step_period == 0):
                if sample_step_by_ddg or sample_step_by_hydro:
                    ...
                    metric_sampled, store_noise, store_v, store_p, store_s = dict(), [], [], [], []
            ...
            v_pred, R_pred, eps_p_pred, c_denoised, res_dim = self.eps_net(
                v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, return_res_dim=True,
            )   # (N, L, 3), (N, L, 3, 3), (N, L, 3)

            for _ in pbar_internal(range(step_sample_times)):  # 1 or 20 (sample by metric)
                if (opt_type == 'Latent') and (compute_ddg_step or compute_hydro_step):
                    # sigma = self.trans_pos.var_sched.sigmas[t]
                    sigma = 1
                    if _ == 0:  # The first noise is fixed as zero
                        noise = torch.zeros(size=res_dim).to(res_feat.device)
                    else:
                        # noise = torch.randn(size=res_dim).to(res_feat.device)
                        ## Uniform distribution
                        noise = torch.rand(size=res_dim) * 2 - 1 
                        noise = noise.to(res_feat.device)
                        # ## Latin Hypercube Sampling
                        # from scipy.stats import qmc            
                        # sampler = qmc.LatinHypercube(d=res_dim[1:].numel())
                        # sample = sampler.random(n=res_dim[0])  # 采样 [0,1] 区间
                        # noise = torch.tensor(qmc.scale(sample, -1, 1), dtype=res_feat.dtype, device=res_feat.device)  # 映射到 [-1,1]
                        # noise = noise.view(res_dim)
                        mask_expanded = mask_generate.unsqueeze(-1)
                        noise = noise*mask_expanded
                        v_pred, R_pred, eps_p_pred, c_denoised = self.eps_net(
                            v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=sigma*noise
                        )   # (N, L, 3), (N, L, 3, 3), (N, L, 3)
                    store_noise.append(noise)
                # DDIM
                v_next = self.trans_rot.denoise(v_t, v_pred, mask_generate, t_tensor, temp_iter)
                p_next = self.trans_pos.denoise(p_t, eps_p_pred, mask_generate, t_tensor, temp_iter)
                _, s_next = self.trans_seq.denoise(s_t, c_denoised, mask_generate, t_tensor) # DDPM
            ...
            if compute_ddg_step or compute_hydro_step:
                if len(metric_sampled) == 1:
                        metric_sampled = torch.stack([torch.stack(item).T for item in metric_sampled.values()]).sum(0)  # batch x num samples
                else:
                    ### weight sum of objectives for MOO
                    if normalize == "Normalize":             
                        # Stack and normalize each metric
                        metric_stacked = torch.stack([torch.stack(item).T for item in metric_sampled.values()])  # objectives x batch x num_samples
                    
                        # Normalize each metric to [0,1] range
                        metric_mean = metric_stacked.mean(dim=-1, keepdim=True)
                        metric_std = metric_stacked.std(dim=-1, keepdim=True)
                        metric_normalized = (metric_stacked - metric_mean) / (metric_std + 1e-8)
                        
                        # Apply weights and sum
                        weights = torch.tensor([property_weight, 1-property_weight]).to(metric_stacked.device)
                        weights = weights.view(-1, 1, 1)  # Shape: (objectives, 1, 1)
                        metric_sampled = 2 * (metric_normalized * weights).sum(0)  # batch x num_samples 
                    elif normalize == "no_Normalize":
                        ### weight sum of objectives for MOO                
                        weights = torch.tensor([property_weight, 1-property_weight]).to(next(iter(metric_sampled.values()))[0].device)  # 权重，形状为 (3,)
                        weights = weights.view(-1, 1, 1)
                        metric_weighted = torch.stack([torch.stack(item).T for item in metric_sampled.values()])  # batch x num_samples
                ...
                if opt_type == "BestN":
                    ...
                    store_noise = torch.stack(store_noise)
                    store_s = torch.stack(store_s)
                    if (sample_step_mode == 'min') or (sample_step_mode == 'exp') or (sample_step_mode == "softmax"):
                    
                        if sample_step_mode == 'min':
                            _, selected = torch.min(metric_sampled, dim=1)                    
                                            
                        elif sample_step_mode == 'exp':   # Get probabilities (using exponential distribution)
                            metric_sampled_min = torch.abs(metric_sampled.min(dim=-1, keepdim=True).values)
                            metric_sampled_shift = metric_sampled + metric_sampled_min                    
                            lambdas = (1 / metric_sampled_min)
                            exp_distrib = lambdas * torch.exp(-temp * lambdas * metric_sampled_shift)
                            prob = exp_distrib / (exp_distrib.sum(dim=-1, keepdim=True) + 1e-8)
                            selected = self._sample_multinomial(prob)
                        elif sample_step_mode == "softmax":  # Get probabilities (using softmax) and sample                   
                            prob = F.softmax(-temp * metric_sampled, dim=-1)
                            selected = self._sample_multinomial(prob)
                        else:
                            raise NotImplementedError
                        print(sample_step_mode)
                        num_idx = torch.arange(len(selected), device=selected.device)
                        row_mins = metric_sampled[num_idx, selected]
                        print(f"ID:{t}, all, row_min:{torch.mean(row_mins):.4g}, row_std:{torch.std(row_mins):.4g}")
                        selected_noise = store_noise[selected, num_idx]
                        ...
                        v_pred, R_pred, eps_p_pred, c_denoised = self.eps_net(
                            v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=sigma*selected_noise
                            )   # (N, L, 3), (N, L, 3, 3), (N, L, 3)
                        # DDIM
                        v_next = self.trans_rot.denoise(v_t, v_pred, mask_generate, t_tensor, temp_iter)
                        p_next = self.trans_pos.denoise(p_t, eps_p_pred, mask_generate, t_tensor, temp_iter)
                        s_next = selected_s ## DDIM
                        if not sample_structure:
                            v_next, p_next = v_t, p_t
                        if not sample_sequence:
                            s_next = s_t
                    ...
                    elif (sample_step_mode == 'tanh') or (sample_step_mode == 'tans'):
                        non_mask_noise_dim = torch.sum(mask_expanded[0]==1)*noise.shape[-1]
                        metric_sampled_mean = torch.mean(metric_sampled, dim=1, keepdims=True)
                        metric_sampled_std = torch.std(metric_sampled, dim=1, keepdims=True)
                        metric_sampled_nor = (metric_sampled-metric_sampled_mean)/(metric_sampled_std+1e-8)
                        # metric_sampled_min = torch.min(metric_sampled, dim=1, keepdims=True)[0]
                        # metric_sampled_max = torch.max(metric_sampled, dim=1, keepdims=True)[0]
                        # metric_sampled_nor = (metric_sampled-metric_sampled_min)/(metric_sampled_max-metric_sampled_min)
                        if sample_step_mode == 'tanh':
                            metric_sampled_nor = torch.tanh(-metric_sampled_nor)  
                        elif sample_step_mode == 'tans':
                            metric_sampled_nor = torch.softmax(-metric_sampled_nor/0.1, dim=1)
                        else:
                            raise NotImplementedError
                        # weight sum noise
                        metric_sampled_nor = metric_sampled_nor.transpose(1, 0).unsqueeze(2).unsqueeze(3)
                        weighted_noise = torch.sum(store_noise * metric_sampled_nor, dim=0)
                        weighted_noise = weighted_noise/(((weighted_noise**2).sum(dim=[1, 2], keepdims=True)).sqrt()+1e-6)*(non_mask_noise_dim**0.5)
                        weighted_noise = weighted_noise.to(store_noise.dtype)
                        
                        v_pred_w, R_pred_w, eps_p_pred_w, c_denoised_w = self.eps_net(v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=sigma*weighted_noise)   # (N, L, 3), (N, L, 3, 3), (N, L, 3)
                        
                        # DDIM
                        v_next_w = self.trans_rot.denoise(v_t, v_pred_w, mask_generate, t_tensor, Iter=101)
                        p_next_w = self.trans_pos.denoise(p_t, eps_p_pred_w, mask_generate, t_tensor, Iter=101)
                        _, s_next_w = self.trans_seq.denoise(s_t, c_denoised_w, mask_generate, t_tensor) ## DDPM
                        
                        if not sample_structure:
                            v_next_w, p_next_w = v_t, p_t
                        if not sample_sequence:
                            s_next_w = s_t
                        p_next_w = self._unnormalize_position(p_next_w)
                        ...
                        if mix != 'false':
                            ...
                            if compute_ddg_step:
                                # Compute predicted ddG with respect to previous states
                                _, p_prev, s_prev = traj[t][:3]
                                metric_.append(predict_ddg_batch(batch_ref, s_next_w, p_next_w, s_prev.to(self._dummy.device), p_prev.to(self._dummy.device)))

                            if compute_hydro_step:
                                # Compute average hydropathy score from generated sequence
                                hydro_tensor = torch.tensor(hydropathy_vector, device=s_next.device)
                                s_cdr = s_next_w[mask_generate].reshape(N, -1)  # select cdr
                                s_hydro = hydro_tensor[s_cdr]  # convert residues to hydro values
                                metric_.append(s_hydro.mean(-1))
                                                        
                            if compute_hydro_step and compute_ddg_step:
                                metric_ = property_weight*metric_[0]+(1-property_weight)*metric_[1]
                            else:
                                metric_ = metric_[0]
                                                            
                            # select the minimum
                            metric_min = torch.min(metric_sampled, dim=1)[0]  # metric_sampled_ shape: b*n
                            if mix == 'min':
                                _, selected = torch.min(metric_sampled, dim=1)                                
                            elif mix == "softmax":  # Get probabilities (using softmax) and sample                   
                                prob = F.softmax(-temp * metric_sampled, dim=-1)
                                selected = self._sample_multinomial(prob)
                            else:
                                raise NotImplementedError
                            ...
                            selected_noise = store_noise[selected, num_idx]
                            selected_s = store_s[selected, num_idx]     
                            v_pred_min, R_pred_min, eps_p_pred_min, c_denoised_min = self.eps_net(v_t, p_t, s_t, res_feat, pair_feat, beta, mask_generate, mask_res, res_feat_noise=sigma*selected_noise)   # (N, L, 3), (N, L, 3, 3), (N, L, 3)
                            ## DDIM
                            v_next_min = self.trans_rot.denoise(v_t, v_pred_min, mask_generate, t_tensor, Iter=101)
                            p_next_min = self.trans_pos.denoise(p_t, eps_p_pred_min, mask_generate, t_tensor, Iter=101)
                            s_next_min = selected_s ## DDIM
                            if not sample_structure:
                                v_next_min, p_next_min = v_t, p_t
                            if not sample_sequence:
                                s_next_min = s_t
                                
                            p_next_min = self._unnormalize_position(p_next_min)
                            
                            p_next = torch.where(metric_[:, None, None] < metric_min[:, None, None], p_next_w, p_next_min) 
                            v_next = torch.where(metric_[:, None, None] < metric_min[:, None, None], v_next_w, v_next_min) 
                            s_next = torch.where(metric_[:, None] < metric_min[:, None], s_next_w, s_next_min) 
                            # metric_max = torch.max(metric_sampled_, dim=1)[0]
                            metric_0 = metric_sampled[:, 0]
                            # print('weighted metric: ', metric_)    
                            # print('min_metric: ', metric_min)      
                            # print('max_metric: ', metric_max)  
                            # print('metric 0: ', metric_0)    
                            print('weighte beter than min', torch.sum(metric_<metric_min)) 
                            print('weighted better than init', torch.sum(metric_<metric_0))    
                            print('min better than init', torch.sum(metric_min<metric_0))     
                        else:
                            p_next = p_next_w
                            v_next = v_next_w
                            s_next = s_next_w
                ...
                if compute_ddg_step:
                    # Compute predicted ddG with respect to previous states
                    _, p_prev, s_prev = traj[t][:3]
                    batch_ddG = predict_ddg_batch(batch_ref, s_next, p_next, s_prev.to(self._dummy.device), p_prev.to(self._dummy.device))
                    print(f"ID:{t}, ddG, row_min:{torch.mean(batch_ddG):.4g}, row_std:{torch.std(batch_ddG):.4g}")
                ...
                if compute_hydro_step:
                    ...
                    s_cdr = s_next[mask_generate].reshape(N, -1)  # select cdr
                    s_hydro = hydro_tensor[s_cdr]  # convert residues to hydro values
                    print(f"ID:{t}, hyd, row_min:{torch.mean(torch.mean(s_hydro, dim=1)):.4g}, row_std:{torch.std(torch.mean(s_hydro, dim=1)):.4g}")
