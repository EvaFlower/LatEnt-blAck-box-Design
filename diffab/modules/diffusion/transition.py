...
class PositionTransition(nn.Module):
    ...
    def denoise(self, p_t, eps_p, mask_generate, t, Iter=101, return_noise=False, z=None):
        # IMPORTANT:
        ...
        if z is None:
            z = torch.where(
                (t >= Iter)[:, None, None].expand_as(p_t),
...
class RotationTransition(nn.Module):
    ...
    def denoise(self, v_t, v_next, mask_generate, t, Iter=101):
        N, L = mask_generate.size()
        e = random_normal_so3(t[:, None].expand(N, L), self.angular_distrib_inv, device=self._dummy.device) # (N, L, 3) ## 可以在这步加噪音
        e = torch.where(
            (t >= Iter)[:, None, None].expand(N, L, 3),
            e, 
...
class AminoacidCategoricalTransition(nn.Module):
    ...
    def __init__(self, num_steps, num_classes=20, var_sched_opt={}, prior_b=0):
        ...
        self.var_sched = VarianceSchedule(num_steps, **var_sched_opt)
        self.prior_b = prior_b # weight of prior distributions # (b=0: uniform distribution; b=1: hydropathy distribution)
