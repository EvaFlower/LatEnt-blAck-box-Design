...
def main():
    ...
    parser.add_argument('-c', '--config', type=str, default='./configs/test/codesign_single_ddg.yml')
    parser.add_argument('-o', '--out_root', type=str, default='./results')
    parser.add_argument('-t', '--tag', type=str, default='partial')
    parser.add_argument('--opt_type', type=str, default='Latent')
    ...
    config.sampling.property_weight = args.property_weight
    config.sampling.normalize = args.normalize
    ...
    type_postfix = '_%s' % args.opt_type if args.opt_type else ''
    mode_postfix = '_%s' % args.sample_step_mode if args.sample_step_mode else ''
    weight_postfix = '_%s' % args.property_weight if args.property_weight else ''
    normalize_postfix = '_%s' % args.normalize if args.normalize else ''
    start_iter_postfix = '_%s' % args.start_iter if args.start_iter else ''
    sample_step_num_postfix = '_%s' % args.sample_step_num if args.sample_step_num else ''
    mix_postfix = '_%s' % args.mix if args.mix else ''
    # log_dir = get_new_log_dir(os.path.join(args.out_root,  'MOO' + type_postfix, config_name + tag_postfix + mode_postfix + weight_postfix + normalize_postfix), prefix='%04d_%s' % (args.index, structure_['id']))
    log_dir = get_new_log_dir(os.path.join(args.out_root, 'Ablation', config_name + tag_postfix + type_postfix + mode_postfix + mix_postfix), prefix='%s' % (structure_['id']))
    print(log_dir)
        ...
        for batch in tqdm(loader, desc=variant['name'], dynamic_ncols=True):
            ...
            if 'abopt' in config.mode:
                ...
                traj_batch = model.sample(batch, sample_opt={
                    ...
                    'property_weight': config.sampling.property_weight,
                    'normalize': config.sampling.normalize,
