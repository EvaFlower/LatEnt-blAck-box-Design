...
def design_for_pdb(args):
    ...
    if (config_name == 'codesign_single') & (args.prior_b == 0):
        tag_postfix = 'uncond'
    elif  (config_name == 'codesign_single') & (args.prior_b > 0):
        tag_postfix = 'AAcond'
    else:
        tag_postfix = args.sample_step_mode
    
    log_dir = get_new_log_dir(
        os.path.join(args.out_root, config_name + '_' + tag_postfix), 
        prefix=data_id
    )
...
def args_from_cmdline():
    ...
    parser.add_argument('-b', '--batch_size', type=int, default=35)
    parser.add_argument('--prior_b', type=float, default=0)
    parser.add_argument('--save_traj', action='store_true', default=False)
    parser.add_argument('--sample_step_mode', type=str, default='softmax')
